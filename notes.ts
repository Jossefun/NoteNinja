...
export const LEVEL_CONFIG: Record<Exclude<LevelKey, 'sensei'>, LevelConfig> = {
...
// Build a pool for Sensei level from user config
export function buildSenseiPool(octaves: Octave[], includeFlats: boolean): NoteCard[] {
  return octaves.flatMap((oct) => [
    ...NATURALS.map((n) => makeNote(n, oct)),
    ...(includeFlats ? FLATS.map((n) => makeNote(n, oct)) : []),
  ]);
}
